//! DvorakJ scan-code → KeyCode conversion table.
...
pub(crate) fn keycode_from_scancode(code: u32) -> Option<KeyCode> {
    match code {
        ...
        0x18 => Some(KeyCode::O),
        0x19 => Some(KeyCode::P),
        0x1A => Some(KeyCode::LBracket),
        0x1B => Some(KeyCode::RBracket),
        0x1E => Some(KeyCode::A),
        0x1F => Some(KeyCode::S),
        0x20 => Some(KeyCode::D),
        0x21 => Some(KeyCode::F),
        ...
        0x73 => Some(KeyCode::Backslash), // JIS "\_" key
        0x7D => Some(KeyCode::Yen),       // JIS "\|" key
        _ => None,
    }
}
